...
func run() {
    ...
    if let idx = args.firstIndex(of: "--session"), args.count > idx + 1 {
        ...
        switch detectSessionSource(url) {
        case .pi:
            do {
...
func detectSessionSource(_ url: URL) -> SessionSourcePath {
    let path = url.path
    if path.contains(".pi/") { return .pi }
    if path.contains(".claude/") { return .claude }
    if path.contains("rollout-") { return .codex }
    if path.contains("state.vscdb") { return .cursor }
    return .unknown
}
